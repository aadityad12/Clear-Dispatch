...
async def ring_call(transcript_id: str) -> str:
    """Create a RINGING call from a transcript_id and return the call_id.

    Used by demo.py to pre-stage a call that the dispatcher can then accept.
    """
    transcript_path = os.path.join(_TRANSCRIPTS_DIR, f"{transcript_id}.json")
    ...
    _log.info("RING_CALL [%s] created — transcript_id=%s", call_id, transcript_id)
    return call_id


@router.post("/call/ring")
async def ring_call_endpoint(body: RingRequest):
    """Stage an incoming call as RINGING so the dispatcher can answer it."""
    try:
        call_id = await ring_call(body.transcript_id)
...
async def accept_call(body: AcceptRequest):
    """Dispatcher answers a RINGING call — sets it ACTIVE and starts transcript streaming."""
    call = next((c for c in state.call_queue if c["id"] == body.call_id), None)
    if call is None:
    ...
    if transcript_data:
        asyncio.create_task(_stream_transcript(body.call_id, transcript_data))

    _log.info("ACCEPT_CALL [%s] — dispatcher answered, status=ACTIVE", body.call_id)
    return {"call_id": body.call_id, "status": "ACTIVE"}
